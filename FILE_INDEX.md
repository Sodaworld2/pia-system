# PIA System — File Index

> **Last Updated:** 2026-02-19
> **Maintained by:** AI agents (see CLAUDE.md rules)
> **Purpose:** Complete index of every `.md` and `.html` file in the repository (excluding `node_modules/`)

---

## How to Use This Index

- Before creating a new file, check if one already exists for that purpose
- When you create or delete a `.md` or `.html` file, update this index
- Categories help agents and humans find what they need fast

---

## Architecture & Design

| File | Purpose |
|------|---------|
| `SITE_MAP.md` | **Complete system inventory** — All 31 HTML pages, every API route, all 37 WebSocket events, 30+ SQLite tables, agent architecture |
| `PIA_ARCHITECTURE.md` | **Single source of truth** — Hub/spoke design, modes, registration, remote spawning |
| `ELECTRON_APP_ANALYSIS.md` | Full Electron desktop app technical analysis (9 chapters) |
| `FULL_FEATURE_SPEC.md` | Complete feature specification |
| `implementation_plan.md` | Implementation plan |
| `ANALYSIS_AND_RECOMMENDATIONS.md` | System analysis and recommendations |
| `DAO_FOUNDATION_SPEC.md` | DAO system specification |
| `DAO_STATE_MACHINES.md` | DAO state machine definitions |
| `DAO_API_CONTRACTS.md` | DAO API contracts and interfaces |
| `THREE-MACHINE-SIMULATION.md` | Three-machine fleet simulation plan |
| `REMOTE_CONTROL_WIREFRAMES.md` | Remote control wireframe descriptions |

## Knowledge & Documentation

| File | Purpose |
|------|---------|
| `CLAUDE.md` | **AI agent instructions** — Rules, architecture, tools, journaling |
| `PIA_KNOWLEDGE_BASE.md` | **Master knowledge base** — Terminology, ideas, spec, capabilities, to-do |
| `FILE_INDEX.md` | **This file** — Index of every .md and .html |
| `KNOWLEDGE_BASE.md` | Legacy knowledge base (older version) |
| `README.md` | Repository README |
| `START_HERE.md` | Getting started guide |
| `QUICKSTART.md` | Quick start instructions |
| `EASY_START_GUIDE.md` | Simplified start guide |
| `USER_GUIDE.md` | User guide |
| `USAGE.md` | Usage documentation |
| `HANDOFF.md` | Agent handoff document |
| `PROGRESS.md` | Progress tracking |

## Session Journals

| File | Purpose |
|------|---------|
| `SESSION_JOURNAL_FETCHED.md` | SodaWorld v2 — Feb 11 research & planning |
| `SESSION_JOURNAL_2026-02-13.md` | Agent Souls + Fisher2050 build |
| `SESSION_JOURNAL_2026-02-14.md` | Mission Control wired to backend + PTY fix |
| `SESSION_JOURNAL_2026-02-15.md` | DAO Admin Dashboard plan + multi-machine agent control |
| `SESSION_JOURNAL_2026-02-16.md` | Permission architecture, features, fleet deployment, Electron, Cortex (13 sessions) |
| `SESSION_JOURNAL_2026-02-16_claude_code.md` | Machine linking, ESM bug fix, all 3 machines online, Cortex Phase 1 |
| `SESSION_JOURNAL_2026-02-17.md` | Security remediation, remote agent spawning, project registry, Playwright MCP, Context7 audit |
| `SESSION_JOURNAL_2026-02-20.md` | **Today** — Multi-machine fixes, video research, full system vision planning, storyboard, parallel agents (5 sessions) |
| `JOURNAL_2026-02-10.md` | Early journal |
| `JOURNAL_2026-02-11.md` | Early journal |
| `JOURNAL-2026-02-11.md` | Early journal (duplicate naming) |
| `JOURNAL_2026-02-12.md` | Journal |
| `JOURNAL_2026-02-12_machine3.md` | Machine 3 specific journal |
| `JOURNAL_2026-02-13.md` | Journal |
| `SESSION_SUMMARY_2026-02-12_hub.md` | Hub session summary |
| `PIA_IMPROVEMENTS_JOURNAL.md` | Improvements journal |
| `PIA_MISSION_CONTROL_DISCUSSION_2026-02-13.md` | Mission Control discussion notes |
| `DAO_DECISION_JOURNAL.md` | DAO decision log |

## Agent Prompts & Briefings

| File | Purpose |
|------|---------|
| `AGENT_BRIEFING.md` | General agent briefing |
| `AGENT_HANDOFF.md` | Agent-to-agent handoff template |
| `AGENT_PROMPT_CORTEX_AI_BRAIN.md` | Prompt: Design the Cortex fleet intelligence brain |
| `AGENT_PROMPT_DAO_SEPARATION.md` | Prompt: Separate DAO into its own repo |
| `AGENT_PROMPT_DEPLOY_FLEET.md` | Prompt: Deploy PIA across 3-machine fleet |
| `AGENT_PROMPT_ELECTRON_APP_EXPLORATION.md` | Prompt: Explore Electron desktop app |
| `AGENT_PROMPT_FIX_GITHUB_PUSH.md` | Prompt: Fix GitHub push block (resolved) |
| `AGENT_PROMPT_MACHINE_3_DAO_RECOVERY.md` | Prompt: Recover DAO on Machine 3 |
| `AGENT_PROMPT_MACHINE_MESSAGING.md` | Prompt: Build machine message board |
| `AGENT_PROMPT_VISION_PRO_EXPLORATION.md` | Prompt: Vision Pro spatial view research |
| `MISSION_CONTROL_AGENT_BRIEFING.md` | Mission Control specific briefing |
| `DAO_AGENT_BRIEFING.md` | DAO agent briefing |
| `M2_ACTIVATION.md` | **CURRENT** — M2 activation guide: identity, services, steps, troubleshooting (Feb 21) |
| `setup-machine.sh` | **NEW** — Run once per machine: auto-detects hostname, creates MACHINE_IDENTITY.local.md |
| `MACHINE_2_SETUP_BRIEFING.md` | Machine 2 (soda-monster-hunter) setup instructions (older) |
| `MACHINE_2_INSTRUCTIONS.md` | Machine 2 detailed instructions (v2) |
| `MACHINE_3_INSTRUCTIONS.md` | Machine 3 detailed instructions (v2) |
| `MACHINE_SETUP_GUIDE.md` | General machine setup guide |
| `MACHINE-2-SETUP.md` | Machine 2 setup (older version) |
| `MACHINE-3-SETUP.md` | Machine 3 setup (older version) |
| `MESSAGE_FROM_HUB.md` | Hub-to-spoke message template |
| `PERMISSION_FIX_HANDOFF.md` | Permission fix handoff doc |

## Plans & Tickets

| File | Purpose |
|------|---------|
| `V1_DEFINITION.md` | **PIA v1.0 definition of done** — 12 acceptance criteria, 17 build items, 5-week target, explicit v2.0 deferral list |
| `SPRINT_PLAN.md` | Sprint plan |
| `PROJECT_PLAN_AGENT_SHOPS.md` | Agent Shops project plan |
| `AGENT_TASK_PLANS.md` | Agent task plans |
| `TICKETS.md` | Issue tickets |
| `REMOTE_SETUP.md` | Remote setup instructions |
| `REMOTE-CLAUDE-SETUP.md` | Remote Claude setup |
| `task.md` | Task tracking |

## Research

| File | Purpose |
|------|---------|
| `AI_AGENT_FRAMEWORKS_RESEARCH_2025_2026.md` | Competitive landscape: AutoGen, CrewAI, LangGraph, etc. |
| `research/MULTI-MACHINE-COMPETITIVE-ANALYSIS.md` | Multi-machine orchestration comparison |
| `research/AGENCY-AGENTS-SALES-COPY.md` | Agency agents sales copy |
| `research/SODALABS-CREATIVE-DECK.md` | SodaLabs creative deck |
| `research/SDK-RESEARCH-REFERENCES.md` | SDK research references |
| `research/AGENT-REVOLUTION-INTEL-2026-02-14.md` | Agent revolution intelligence report |
| `research/agreements-signatures-research.md` | Agreement signatures research |
| `research/ORCHESTRATOR-AGENT-VIDEO-SYNTHESIS.md` | Full synthesis: Orchestrator Agent lesson (Core 4, CRUD for agents, observability, delete philosophy, PIA build checklist) |
| `research/CLAUDE-CODE-MULTI-AGENT-NATIVE-SYNTHESIS.md` | Full synthesis: Claude Code native multi-agent tools (team_create, task_*, send_message, agent sandboxes, E2B) |
| `research/MASTER_VISION.md` | **Comprehensive vision document** — full system diagram, all agent specs (12 agents), 5 architectural patterns, intelligence pipeline, messaging system, build order |
| `research/AGENT_PRODUCT_SHEETS.md` | Internal product sheet per agent — soul, trigger, inputs, outputs, training notes (for AI training) *(created by sub-agent Session 5)* |
| `research/BUILD_LIST.md` | **Master build list** — 53 items across 7 priority tiers, dependency graph, quick wins *(created Session 8)* |
| `research/DOCUMENT_INDEX.md` | Index of all documents in InvestorDome, sodalabs, sodaworld, and PIA research folders |
| `research/USER_WORKING_STYLE.md` | Agent training rules + working style analysis — 9 methodology frameworks, Mic's communication rules |
| `research/ELIYAHU_END_OF_DAY_SPEC.md` | **Eliyahu EOD Agent spec** — how agent querying works (3 scenarios), async inbox pattern, full Eliyahu implementation blueprint, 10-step build order |
| `research/IMPLEMENTATION_SPEC.md` | **Builder's blueprint** — Fisher2050 merge spec, Google Calendar OAuth, Google Tasks sync, 3 new DB tables, 8 new API endpoints, TypeScript interfaces |
| `research/DEVILS_ADVOCATE.md` | **Critical analysis** — 6 architectural risks, complexity risks, business contradictions, 6 immediate fixes, technique dictionary with failure modes |
| `research/SYNC_REPORT.md` | **Knowledge sync report** — 20 gaps found, 8 new pia-book.html chapters added, outstanding items and HTML page status *(Session 12, 2026-02-20)* |
| `research/CURRENT_STATE_AUDIT.md` | **Deep technical audit** — every src/ file read, 28 working capabilities, 7 activation priorities, what's wired vs stub *(Session 26, 2026-02-20)* |
| `research/CODE_REPOSITORY_MAP.md` | **Complete code ecosystem map** — 26 codebases mapped, 14 with GitHub remotes, stacks, status, relation to PIA |
| `research/TERMINAL_SEARCH_BRIEFING.md` | **Self-contained prompt for new terminal** — paste into fresh Claude to do deep repo search + produce unification plan |

## Data & Knowledge Store

| File | Purpose |
|------|---------|
| `data/context7-research.md` | Context7 MCP research findings |
| `data/context7-libs-audit.md` | Library audit via Context7 (8 libraries) |
| `data/knowledge/README.md` | Knowledge store README |
| `data/knowledge/remote-fleet-management.md` | Remote fleet management knowledge |
| `data/knowledge/machine-setup-guide.md` | Machine setup knowledge |
| `data/knowledge/meshcentral-sidecar-research.md` | MeshCentral sidecar research |
| `data/knowledge/resistance-points-research.md` | Resistance points research |
| `data/knowledge/per-machine-checklist.md` | Per-machine checklist |

## Martin's Template (Reference)

| File | Purpose |
|------|---------|
| `Martin/claude-orchestration-template-main/README.md` | Orchestration template README |
| `Martin/claude-orchestration-template-main/CHANGELOG.md` | Template changelog |
| `Martin/claude-orchestration-template-main/CONTRIBUTING.md` | Contribution guide |
| `Martin/claude-orchestration-template-main/SYSTEM_ANALYSIS.md` | System analysis |
| `Martin/claude-orchestration-template-main/DAOV1_CLAUDE.md` | DAOV1 Claude instructions |
| `Martin/claude-orchestration-template-main/SESSION_JOURNAL.md` | Template session journal |
| `Martin/claude-orchestration-template-main/VIDEOHOHO-MCP-RECOMMENDATIONS.md` | MCP recommendations |
| `Martin/claude-orchestration-template-main/docs/CATALOGUE.md` | Template catalogue |
| `Martin/claude-orchestration-template-main/docs/MAINTENANCE.md` | Maintenance guide |
| `Martin/claude-orchestration-template-main/docs/WORKFLOW.md` | Workflow guide |

---

## HTML Files — Dashboard & UI

| File | Purpose | Served At |
|------|---------|-----------|
| `public/mission-control.html` | **Main dashboard** — Agent control, terminal, journal (~3000 lines) | `/mission-control.html` |
| `public/mission-control-cli.html` | CLI version of Mission Control | `/mission-control-cli.html` |
| `public/visor.html` | Visor panel | `/visor.html` |
| `public/index.html` | Landing page | `/` |
| `public/pia-admin.html` | PIA admin panel | `/pia-admin.html` |
| `public/pia-book.html` | **Project knowledge book** (visual HTML guide) | `/pia-book.html` |
| `public/first-run.html` | Electron first-run setup wizard | `/first-run.html` |
| `public/settings.html` | Electron settings page | `/settings.html` |

## HTML Files — Guides & Documentation

| File | Purpose | Served At |
|------|---------|-----------|
| `public/guide.html` | PIA guide | `/guide.html` |
| `public/handbook.html` | PIA handbook | `/handbook.html` |
| `public/simple-guide.html` | Simplified guide | `/simple-guide.html` |
| `public/how-it-works.html` | How PIA works | `/how-it-works.html` |
| `public/how-claude-learns.html` | How Claude learns | `/how-claude-learns.html` |
| `public/knowledge.html` | Knowledge page | `/knowledge.html` |
| `public/terminology.html` | Terminology page | `/terminology.html` |
| `public/checklist.html` | Checklist | `/checklist.html` |
| `public/showcase.html` | Showcase | `/showcase.html` |
| `public/work.html` | Work page | `/work.html` |
| `public/offline.html` | Offline fallback page | `/offline.html` |

## HTML Files — Planning & Design

| File | Purpose | Served At |
|------|---------|-----------|
| `public/system-plan.html` | System plan visualization | `/system-plan.html` |
| `public/pia-plan-infographic.html` | PIA plan infographic | `/pia-plan-infographic.html` |
| `public/pia-plan.html` | Visual planning doc (pizza analogy, M1/M2 arch, research) | `/pia-plan.html` |
| `public/pia-diagram.html` | **Architecture diagram** — Now vs After, to-do list, agent ecosystem | `/pia-diagram.html` |
| `public/pia-mindmap.html` | **Interactive D3.js mind map** — full PIA × SodaLabs architecture, zoom/pan, click for detail | `/pia-mindmap.html` |
| `public/pia-storyboard.html` | **15-scene Club X storyboard** — all agents working, live queue panel, auto-play | `/pia-storyboard.html` |
| `public/wireframes.html` | Wireframe designs | `/wireframes.html` |
| `public/agent-generator.html` | Agent generator tool | `/agent-generator.html` |

## HTML Files — DAO

| File | Purpose | Served At |
|------|---------|-----------|
| `public/dao-dashboard.html` | DAO dashboard | `/dao-dashboard.html` |
| `public/dao-login.html` | DAO login page | `/dao-login.html` |
| `public/dao-tokens.html` | DAO tokens page | `/dao-tokens.html` |
| `public/dao-voting.html` | DAO voting page | `/dao-voting.html` |

## HTML Files — Root-Level Mockups

| File | Purpose |
|------|---------|
| `FLEET_DASHBOARD_MOCKUP.html` | Fleet dashboard mockup ("Empire Fleet Dashboard") |
| `MASTER_DASHBOARD.html` | Master dashboard mockup |
| `PIA_DASHBOARD_MOCKUP.html` | PIA dashboard mockup |
| `SIMPLE_DASHBOARD_MOCKUP.html` | Simple dashboard mockup |
| `CLI_TUNNEL_MOCKUP.html` | CLI tunnel mockup |
| `STORYBOARD.html` | Storyboard visualization |
| `DEVELOPMENT_ROADMAP.html` | Development roadmap visualization |
| `wireframes/remote-control-mockup.html` | Remote control wireframe mockup |

## HTML Files — Other

| File | Purpose |
|------|---------|
| `test-screenshots/TEST_REPORT.html` | Test report with screenshots |
| `fisher2050/public/index.html` | Fisher2050 project manager app |
| `Martin/claude-orchestration-template-main/screenshots-review/system-review.html` | Template system review |
| `dao-foundation-files/research/spaceship-for-nine.html` | DAO research page |

---

## DAO Foundation Files (SEPARATE PROJECT — do not modify)

The `dao-foundation-files/` directory is a separate project. See `CLAUDE.md` for rules.

| File | Purpose |
|------|---------|
| `dao-foundation-files/SPRINT_PLAN_GOVERNANCE_TREASURY_AGREEMENTS_TOKENS.md` | DAO sprint plan |
| `dao-foundation-files/SOLANA_SODA_TOKEN_PLAN.md` | Solana SODA token plan |

---

## Statistics

- **Total .md files (excl. node_modules):** ~80+
- **Total .html files (excl. node_modules):** ~40+
- **Session journals:** 10 files spanning Feb 10-17, 2026
- **Agent prompts:** 10 briefing/prompt documents
- **Research docs:** 6 research documents
- **HTML dashboards/UIs:** 26 pages served by Express
