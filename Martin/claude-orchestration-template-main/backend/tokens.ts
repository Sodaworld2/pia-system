...
router.post('/reward', async (req, res) => {
  try {
    ...
    // Add tokens to user with optimistic locking
    const existingUser = await db('user_balances').where({ user_id: userId }).first();
    let result;
    if (existingUser) {
      result = await incrementBalance(userId, 'soda_balance', amount);
    } else {
    ...
    // Get bubble score
    const userBalance = await db('user_balances').where({ user_id: userId }).first();

    res.json({
      success: true,
      transactionId: txId,
      newBalance: result.newBalance,
      bubbleScore: userBalance.bubble_score
    });
  } catch (error) {
...
// Update bubble score
router.post('/bubble-score', async (req, res) => {
  try {
    ...
    let result;
    if (existingUser) {
      result = await incrementBalance(userId, 'bubble_score', points);
    } else {
      // Create new user account
      await db('user_balances').insert({
        user_id: userId,
    ...
    res.json({
      success: true,
      newBubbleScore: result.newBalance,
      action
    });
  } catch (error) {
    console.error('Error updating bubble score:', error);
    res.status(500).json({ error: 'Failed to update bubble score' });
  }
});
