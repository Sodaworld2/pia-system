...
export class GovernanceModule extends BaseModule {
    ...
    systemPrompt = `You are the Governance module for a DAO (Decentralized Autonomous Organisation) platform called SodaWorld.
...
Always respond in a structured, actionable way. Use markdown formatting.`;
     ...
     */
    async analyzeProposal(daoId, userId, proposalId) {
     ...
     */
    async recommendVotingParameters(daoId, userId, params) {
        const policies = await this.getKnowledge(daoId, 'policy');
        const decisions = await this.getKnowledge(daoId, 'decision');
        const policySummary = policies
            .map((p) => `- ${p.title}: ${p.content}`)
        ...
        const historicalProposals = await this.db('proposals')
            .where({ dao_id: daoId })
        ...
        const votingParams = this.extractVotingParameters(aiResponse.content, params.proposalType, params.memberCount);
        bus.emit({
     ...
     */
    async generateGovernanceReport(daoId, userId, params) {
        ...
        for (const proposal of proposals) {
            ...
            const yesVotes = proposalVotes.filter((v) => v.choice === 'yes').length;
            if (voterCount > 0) {
        ...
        try {
            const delegations = await this.db('delegations')
                ...
                .where('created_at', '>=', periodStart)
                .where('created_at', '<=', periodEnd);
            const delegators = new Set(delegations.map((d) => d.delegator_id));
            const delegates = new Set(delegations.map((d) => d.delegate_id));
            delegationStats = {
        ...
        const metrics = {
            ...
            quorum_achievement_rate: proposals.length > 0
     ...
     */
    async draftConstitution(daoId, userId, params) {
        const policies = await this.getKnowledge(daoId, 'policy');
        const decisions = await this.getKnowledge(daoId, 'decision');
        const precedents = await this.getKnowledge(daoId, 'precedent');
        const policySummary = policies
     ...
     */
    extractGovernanceHints(content) {
        const hints = [];
        ...
        if (lower.includes('precedent') ||
            lower.includes('last time') ||
            lower.includes('previously') ||
            lower.includes('in the past')) {
            const precedentMatch = content.match(/(?:precedent|last time|previously|in the past)[,: ]+(.+?)(?:\.|$)/i);
            if (precedentMatch?.[1]) {
        ...
        if (lower.includes('quorum') || lower.includes('threshold') || lower.includes('supermajority')) {
            const quorumMatch = content.match(/(?:quorum|threshold|supermajority)[: ]+(.+?)(?:\.|$)/i);
            if (quorumMatch?.[1]) {
     ...
     */
    extractImpactAssessment(content) {
        const assessLevel = (keyword) => {
     ...
     */
    extractListItems(content, section) {
        const items = [];
        const sectionPattern = new RegExp(`(?:\\*\\*${section}\\*\\*|## ${section}|### ${section})([\\s\\S]*?)(?=(?:\\*\\*|## |### |$))`, 'i');
     ...
     */
    extractVotingParameters(content, proposalType, memberCount) {
        const extractNumber = (keyword, fallback) => {
        ...
        if (lower.includes('quadratic'))
            recommendedModel = 'quadratic';
        else if (lower.includes('conviction'))
            recommendedModel = 'conviction';
        else if (lower.includes('holographic'))
            recommendedModel = 'holographic';
        else if (lower.includes('token-weighted') || lower.includes('token weighted'))
            recommendedModel = 'token_weighted';
        return {
     ...
     */
    parseConstitutionDraft(content, governanceModel) {
        const articles = [];
        let preamble = '';
        // Extract preamble (text before first article)
        const preambleMatch = content.match(/(?:preamble|^)([\s\S]*?)(?=(?:article\s+[iv\d]+|## article))/i);
        if (preambleMatch?.[1]) {
        ...
        const articlePattern = /(?:#{1,3}\s*)?(?:\*\*)?Article\s+([IVXLCDM]+|\d+)[:.]\s*(.+?)(?:\*\*)?[\n\r]([\s\S]*?)(?=(?:(?:#{1,3}\s*)?(?:\*\*)?Article\s+[IVXLCDM\d]+|$))/gi;
        let match;
        let articleIndex = 1;
        ...
        const amendmentsArticle = articles.find((a) => a.title.toLowerCase().includes('amendment') ||
