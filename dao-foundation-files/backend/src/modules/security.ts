...
interface AccessAuditReport {
  ...
  over_privileged: AccessControlEntry[];
  inactive_admins: AccessControlEntry[];
...
export class SecurityModule extends BaseModule {
  readonly moduleId: AIModuleId = 'security';
...
Format findings with severity levels (critical, high, medium, low).`,
  ...
  async threatModel(
    daoId: string,
    userId: string,
    params: { asset: string; threats: string[] },
  ): Promise<AgentResponse> {
    const policies = await this.getKnowledge(daoId, 'policy');
    const lessons = await this.getKnowledge(daoId, 'lesson');
...
Present findings in a structured table format where possible.`,
  ...
  async incidentResponse(
    daoId: string,
    userId: string,
    params: { incident_type: string; severity: string; description: string },
  ): Promise<AgentResponse> {
    const procedures = await this.getKnowledge(daoId, 'procedure');
    const lessons = await this.getKnowledge(daoId, 'lesson');
...
Be specific and urgent. Prioritise containment actions. For ${params.severity} severity, indicate expected response time SLAs.`,
   ...
   */
  async reviewSmartContract(
    daoId: string,
    userId: string,
    params: { contractAddress: string; code: string },
  ): Promise<AgentResponse> {
    const policies = await this.getKnowledge(daoId, 'policy');
    const lessons = await this.getKnowledge(daoId, 'lesson');
  ...
  private extractKnowledgeHints(
    ...
    if (
      ...
      const lessonMatch = content.match(
        /(?:lesson learned|we discovered|root cause|post-?mortem)[: ](.+?)(?:\.|$)/i,
      );
      if (lessonMatch?.[1]) {
    ...
    if (
      ...
      const vulnMatch = content.match(
        /(?:vulnerability|exploit|breach|compromised)[: ](.+?)(?:\.|$)/i,
      );
      if (vulnMatch?.[1]) {
