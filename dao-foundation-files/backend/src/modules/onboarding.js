...
export class OnboardingModule extends BaseModule {
    ...
    systemPrompt = `You are the Onboarding module for a DAO (Decentralized Autonomous Organisation) platform called SodaWorld.
...
Always respond in a structured, actionable way. Use markdown formatting.`;
     ...
     */
    async generateOnboardingPlan(daoId, userId, params) {
     ...
     */
    async assessReadiness(daoId, userId) {
     ...
     */
    async recommendFirstContribution(daoId, userId, params) {
        const resources = await this.getKnowledge(daoId, 'resource');
        const procedures = await this.getKnowledge(daoId, 'procedure');
        const preferences = await this.getKnowledge(daoId, 'preference');
        const resourceSummary = resources
     ...
     */
    async explainDAO(daoId, userId, params) {
        const resources = await this.getKnowledge(daoId, 'resource');
        const procedures = await this.getKnowledge(daoId, 'procedure');
        const resourceSummary = resources
            .map((r) => `- ${r.title}: ${r.content}`)
     ...
     */
    async saveOnboardingPlan(daoId, userId, plan) {
        const saved = [];
        // Save the overall plan as a procedure
        saved.push(await this.addKnowledge(daoId, {
            category: 'procedure',
            ...
            content: [
                ...
                '',
                'Steps:',
                ...plan.steps.map((s) => `  - [${s.completed ? 'x' : ' '}] ${s.title} (${s.category}, ${s.estimated_minutes}min, ${s.required ? 'required' : 'optional'})`),
            ].join('\n'),
            source: 'ai_derived',
            created_by: userId,
     ...
     */
    async recordStepCompletion(daoId, userId, stepTitle) {
    ...
    extractKnowledgeHints(content) {
        ...
        if (lower.includes('prefer') || lower.includes('rather') || lower.includes('style') || lower.includes('like to')) {
            const prefMatch = content.match(/(?:prefer|rather|style|like to)[: ](.+?)(?:\.|$)/i);
            if (prefMatch?.[1]) {
        ...
        if (lower.includes('experience') || lower.includes('familiar with') || lower.includes('background in')) {
            const expMatch = content.match(/(?:experience (?:with|in)|familiar with|background in) (.+?)(?:\.|$)/i);
            if (expMatch?.[1]) {
