...
export class AnalyticsModule extends BaseModule {
    ...
    systemPrompt = `You are the Analytics module for a DAO (Decentralized Autonomous Organisation) platform called SodaWorld.
...
Always respond in a structured, data-driven way. Use markdown formatting with tables and lists where appropriate.`;
     ...
     */
    async analyzeTrends(daoId, userId, params) {
     ...
     */
    async generateDashboard(daoId, userId, params) {
     ...
     */
    async detectAnomalies(daoId, userId, params) {
        const metrics = await this.getKnowledge(daoId, 'metric');
        const lessons = await this.getKnowledge(daoId, 'lesson');
        const metricSummary = metrics.map((m) => `- ${m.title}: ${m.content}`).join('\n');
        const lessonSummary = lessons.map((l) => `- ${l.title}: ${l.content}`).join('\n');
        ...
        const anomalyTable = anomalies.length > 0
            ? anomalies
                .map((a) => `| ${a.index} | ${a.value.toFixed(2)} | ${a.expected.toFixed(2)} | ${a.deviation.toFixed(2)} | ${a.severity} |`)
                .join('\n')
     ...
     */
    async forecastMetric(daoId, userId, params) {
        const metrics = await this.getKnowledge(daoId, 'metric');
        const decisions = await this.getKnowledge(daoId, 'decision');
        const lessons = await this.getKnowledge(daoId, 'lesson');
        const metricSummary = metrics.map((m) => `- ${m.title}: ${m.content}`).join('\n');
        ...
        const forecastTable = forecast.predictions
            .map((p) => `| +${p.period} | ${p.predicted_value.toFixed(2)} | ${p.confidence_lower.toFixed(2)} | ${p.confidence_upper.toFixed(2)} |`)
            .join('\n');
        return this.processMessage({
            content: [
     ...
     */
    async saveInsight(daoId, userId, insight) {
     ...
     */
    async saveMetricDefinition(daoId, userId, definition) {
        const content = [
     ...
     */
    async recordDecision(daoId, userId, decision) {
        const content = [
    ...
    computeTrendDirection(values) {
        ...
        const changeVariance = changes.reduce((a, b) => a + (b - changeMean) ** 2, 0) / changes.length;
     ...
     */
    computeAnomalies(values, threshold) {
        const anomalies = [];
        if (values.length < 3) {
            return anomalies;
        }
     ...
     */
    computeLinearForecast(values, periods) {
        const n = values.length;
        if (n < 2) {
    ...
    extractAnalyticsHints(content) {
        ...
        if (lower.includes('learned') ||
            lower.includes('insight') ||
            lower.includes('finding') ||
            lower.includes('discovered')) {
            const lessonMatch = content.match(/(?:learned|insight|finding|discovered)[: ](.+?)(?:\.|$)/i);
            if (lessonMatch?.[1]) {
        ...
        if (lower.includes('decided') ||
            ...
            const decisionMatch = content.match(/(?:decided|decision|chose|concluded)[: ](.+?)(?:\.|$)/i);
            if (decisionMatch?.[1]) {
