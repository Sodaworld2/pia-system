...
export class CommunityModule extends BaseModule {
    ...
    systemPrompt = `You are the Community module for a DAO (Decentralized Autonomous Organisation) platform called SodaWorld.
...
Always respond in a structured, actionable way. Use markdown formatting.`;
     ...
     */
    async analyzeEngagement(daoId, userId, params) {
     ...
     */
    async draftAnnouncement(daoId, userId, params) {
        const preferences = await this.getKnowledge(daoId, 'preference');
        const resources = await this.getKnowledge(daoId, 'resource');
        const preferenceSummary = preferences
            .map((p) => `- ${p.title}: ${p.content}`)
     ...
     */
    async resolveConflict(daoId, userId, params) {
        const preferences = await this.getKnowledge(daoId, 'preference');
        const contacts = await this.getKnowledge(daoId, 'contact');
        const preferenceSummary = preferences
            .map((p) => `- ${p.title}: ${p.content}`)
        ...
        const partiesInfo = contacts
            .filter((c) => params.parties.some((party) => c.title.toLowerCase().includes(party.toLowerCase()) ||
     ...
     * and success metrics.
     */
    async planEvent(daoId, userId, params) {
        const resources = await this.getKnowledge(daoId, 'resource');
        const preferences = await this.getKnowledge(daoId, 'preference');
        const contacts = await this.getKnowledge(daoId, 'contact');
        const resourceSummary = resources
     ...
     */
    async saveMemberProfile(daoId, userId, profile) {
        const content = [
     ...
     */
    async saveResource(daoId, userId, resource) {
        const content = [
     ...
     */
    async saveCommunityNorm(daoId, userId, norm) {
        return this.addKnowledge(daoId, {
            category: 'preference',
