...
export class CoachModule extends BaseModule {
    ...
    systemPrompt = `You are the Coach module for a DAO (Decentralized Autonomous Organisation) platform called SodaWorld.
...
Always respond in a structured, actionable way. Use markdown formatting.`;
     ...
     */
    async generateOKRs(daoId, userId, context) {
     ...
     */
    async planMilestones(daoId, userId, objective, timeframeWeeks) {
     ...
     */
    async swotAnalysis(daoId, userId) {
     ...
     */
    async saveCoachingPlan(daoId, userId, plan) {
        const saved = [];
        // Save goals
        ...
        for (const okr of plan.okrs) {
            const content = [
                ...
                `Status: ${okr.status}`,
                'Key Results:',
                ...okr.keyResults.map((kr) => `  - ${kr.description}: ${kr.current}/${kr.target} ${kr.unit}`),
            ].join('\n');
            saved.push(await this.addKnowledge(daoId, {
                category: 'goal',
    ...
    extractKnowledgeHints(content) {
        ...
        if (lower.includes('strength') || lower.includes('good at') || lower.includes('excel at')) {
            const strengthMatch = content.match(/(?:strength|good at|excel at)[: ](.+?)(?:\.|$)/i);
            if (strengthMatch?.[1]) {
        ...
        if (lower.includes('prefer') || lower.includes('rather') || lower.includes('style')) {
            const prefMatch = content.match(/(?:prefer|rather|style)[: ](.+?)(?:\.|$)/i);
            if (prefMatch?.[1]) {
