...
| **Agent Enrichment System** | Planned UI + backend feature for editing, repairing, analysing, and importing/exporting agent souls. Consists of four components: Soul Roster Grid (12 cards), Soul Editor (6-section form), Memory Panel, and System Prompt Preview. No new backend needed — all backed by existing `/api/souls/*` API. Critical pre-requisite: wire `soulId` into `agent-session.ts` spawn flow (~10 lines). |
