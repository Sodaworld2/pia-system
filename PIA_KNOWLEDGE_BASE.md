...
| **Agent Enrichment System** | Planned UI + backend feature for editing, repairing, analysing, and importing/exporting agent souls. Consists of four components: Soul Roster Grid (12 cards with health scores), Soul Editor (6-section form: identity/character/mission/goals/relationships/config), Memory Panel (add/view/search), System Prompt Preview (live render). Lives at `public/soul-enrichment.html` — separate page from mission-control.html. Full spec: `research/SOUL_ENRICHMENT_SPEC.md`. |
| **Soul Health Score** | A 0–100% completeness score calculated per soul. Criteria: personality (200+ chars) +20, system_prompt (300+ chars) +20, goals (3+ items) +20, relationships (3+ entries) +20, config with machine +10, memories (5+) +10. Eliyahu's audit reports this score for all 12 agents. |
| **Soul Wiring Gap** | Critical bug (as of Feb 2026): all 12 souls are seeded and stored in SQLite but `agent-session.ts` never calls `generateSystemPrompt()` before spawning. Every agent spawned via Mission Control is running without their soul. Fix: ~10 lines in `agent-session.ts`. |
